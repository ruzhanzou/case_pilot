...
当前验收版本聚焦“登录 → 管理用例资产 → 创建执行任务 → 多人协作记录结果”
的完整闭环。AI 用例生成、文件分析和 AI 改写暂未在当前产品界面与 OpenAPI
中启用；仓库保留独立 Agent 和 Mock Provider，供后续生成流程接入。
...
| Agent | Python、Celery | 隔离的文件分析、模型 Provider 与生成管线；当前产品流程未启用 |
...
可选：在另一个终端启动独立 Agent。当前产品页面不会提交生成任务，但可单独
验证 Agent 管线：
