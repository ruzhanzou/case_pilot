...
> 日期：2026-07-25
> 状态：开发验收同步稿
> 关联文档：[产品规格书 V1.4](./product-spec-v1.md)、[产品设计 V1.4](./product-design-v1.md)、[产品逻辑 Review V1.4](./product-logic-review-v1.4.md)

## 1. 本次更新结论

V1.4 将产品主流程收敛为一个可恢复、可追踪、必须人工确认的闭环：

`选择空间 → 创建/选择用例集合 → 对话与上传 → 文件解析 → 风险识别 → 测试点生成 → 脑图与文本用例生成 → 编辑/AI 改写 → 人工评审 → 状态记录 → 结构化测试说明 → 发布/导出 → QA 测试执行`
...
- 用例自身状态统一为通过、不通过、Pending、跳过、堵塞；评审人员与意见作为事件历史保存，不再形成第二套用户可见状态。
- 首页和集合工作台共用模型选择；模型选择跟随生成任务而不是消息气泡。
- 正常连接状态不常驻顶部；只在检查中、降级或离线影响当前操作时提供故障反馈。
- 底部浮动控件采用统一安全间距，不贴近浏览器或系统边缘。
- QA 可加载集合逐条执行；本次执行结果独立于用例内容状态，并即时更新运行进度。
...
- 平台保存权威资产、用例状态、版本和审计；本地 Agent 只能提交候选变更。
...
| 依赖环境不可用 | 当前执行标记为堵塞 | 填写依赖与恢复条件 | 解除后回到 Pending |
...
4. 切换成功后，需求、会话、集合、成员、状态筛选和模型配置全部刷新。
...
**层级**：集合/需求基线 → 模块 → 测试点 → 用例。前置、步骤、校验点默认在详情面板展示，避免脑图过密。

**基础交互**：

- 单击选中并打开详情；双击进入标题编辑。
- 滚轮缩放、按住空格拖动画布、快捷键聚焦选中节点。
- 节点支持展开、折叠、搜索定位和一键适应画布。
...
| 查看 | 点击节点打开右侧详情 | 显示来源、版本、评审、执行和自动化绑定 |
...
6. 接受后创建新 Candidate Revision；当前用例状态回到 Pending，原 Published Revision 保持不变。
...
**评审事件**：提交评审、分配评审人、字段级评论、通过、驳回、撤回和发布。评审事件不作为第二套用户可见用例状态。

**流程**：提交评审 → 分配评审人 → 字段级评论/修改建议 → 将用例设置为通过或不通过 → 发布基线。

**规则**：

- 驳回必须填写原因并可指派修改人。
- 批量通过前展示数量、风险和缺失字段，二次确认。
- 已通过内容被修改后生成新修订，新修订状态回到 Pending；旧基线保持只读。
- 每次评审保存人员、时间、结论、评论和所评审的精确版本。

### 4.11 用例状态

五种状态定义为用例本身的当前状态，统一保存为 `case_status`：

| 状态 | 含义 | 必填信息 | 后续动作 |
|---|---|---|---|
| Pending | 新建、AI 生成、导入或修改后等待处理 | 可选负责人和备注 | 可进入其余任一状态 |
| 通过 | 当前用例内容已确认可用 | 操作人、时间 | 内容产生新修订后回 Pending |
| 不通过 | 当前用例内容存在问题 | 不通过原因、问题字段 | 修改后回 Pending |
| 跳过 | 当前范围内明确不处理 | 跳过原因 | 重新纳入范围后回 Pending |
| 堵塞 | 因需求、依赖或其他条件无法继续 | 原因、依赖、解除条件 | 解除后回 Pending |

状态变化写入事件历史，不覆盖旧记录。集合页、脑图节点、用例详情和测试说明均读取同一个最新 `case_status`。评审人员、评论和决定作为 `review_events` 保存，不再展示第二套评审状态。

视觉映射固定为：

| 状态 | 颜色语义 | 非颜色提示 |
|---|---|---|
| Pending | 黄色，等待处理 | 圆点 + “Pending” |
| 通过 | 绿色，确认可用 | 圆点 + “通过” |
| 不通过 | 红色，内容问题 | 圆点 + “不通过” |
| 跳过 | 紫色，本轮不处理 | 圆点 + “跳过” |
| 堵塞 | 橙色，依赖阻断 | 圆点 + “堵塞” |

所有状态标签采用浅色背景、彩色文字、同色圆点和细边框，禁止使用大面积深色填充；颜色只作为辅助信息，文字始终保留。

实现基准色如下，其他页面不得自行定义替代色：

| 状态 | 浅色背景 | 文字 | 边框/圆点 |
|---|---|---|---|
| Pending | `#FFF8E6` | `#A16207` | `#F6D98B` / `#D99A16` |
| 通过 | `#ECFDF3` | `#087A4B` | `#A7E3C3` / `#16A36A` |
| 不通过 | `#FFF1F2` | `#C43343` | `#F5B8BF` / `#E34856` |
| 跳过 | `#F5F1FF` | `#6D45B8` | `#D7C7FA` / `#8B5CF6` |
| 堵塞 | `#FFF4E8` | `#B45309` | `#F5C58D` / `#EA8A23` |
...
- 顶部：用例集合选择、运行保存、执行进度与结果统计。
- 左侧：集合内用例执行队列。
- 右侧：前置条件、执行步骤、校验点、本次执行结果与备注。
...
- 切换用例后保留当前运行内已保存结果。
- 页面统计由 Execution Record 派生，不从用例状态推断。
...
| 维度 | 建议字段 | 典型值 |
|---|---|---|
| 用例当前状态 | `case_status` | pending、passed、failed、skipped、blocked |
| 修订生命周期 | `revision_state` | draft、published、superseded |
| 生成任务状态 | `generation_status` | queued、running、completed、failed、cancelled |

评审人员、评论和通过/驳回动作以事件保存，用于审计和历史展示，不与 `case_status` 形成两套互相冲突的用户状态。
...
- 集合支持通过、不通过、Pending、跳过、堵塞五种用例状态筛选。
- 脑图、列表、详情和测试说明使用同一个 `case_status`。
- 测试说明能追踪到需求、风险、测试点、用例、版本和状态快照。
- 所有关键写操作保存操作者、时间、原因、版本和前后差异。

## 8. Figma 交付索引

- AI 工作台 V2.1：多文件上传、共享模型选择、风险、进度、脑图 CRUD、自然语言节点改写、五状态图例和底部安全间距。
- 完整工作流总览：9 阶段主流程、AI 流水线、人工确认点、平台资产边界与异常分支。
- 功能模块交互：6 个模块、24 个关键状态卡片。
- 用例集合 V2：空间选择、当前空间新建集合和五状态筛选。
- 结构化测试说明：目录、正文、风险、状态矩阵、追踪、版本和导出。
