...
1. 聊天创建页：选择目标集合、输入测试目标、添加本地附件上下文；
2. 用例工作台：展示对话、生成阶段、脑图/列表和结构化详情；候选内容经
   用户点击“写入用例集”后才调用正式用例 API。

创建页与工作台输入框共享模型选择；桌面三栏分隔条可以拖拽或使用方向键
调整宽度。用例资产页的“AI 编辑”直接携带当前集合进入工作台。

本地候选生成用于验证页面和资产门禁，不等同于真实文件解析或模型生成。
后续接入 Generation Job 时保持当前前端状态结构和人工确认边界不变。
