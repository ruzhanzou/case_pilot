...
- 具体结论见 [开源项目调研](./opensource-research.md)；产品体验见 [产品设计 V1.4](./product-design-v1.md)；完整工作流、状态模型与模块交互见 [产品交互设计 V1.4](./product-interaction-design.md)；工程范围、架构、接口和里程碑见 [开发基线](./development-baseline-v1.md)；本轮一致性结论见 [产品逻辑 Review V1.4](./product-logic-review-v1.4.md)。
...
1. QA 从用例集管理页或主导航进入“测试执行”。
2. 选择用例集合并创建本次 Execution Run。
3. 系统冻结集合基线与用例 Revision，加载执行队列。
4. QA 查看前置条件、执行步骤和校验点。
5. QA 逐步确认，并标记本次执行结果。
6. 不通过或堵塞时填写实际结果与缺陷/阻塞项。
7. 页面同步更新队列状态、完成进度和结果统计。

执行结果独立于用例内容状态，不得回写覆盖 `case_status`。
...
操作：缩放、定位、折叠分支、选择、批量选择、新增测试点、展开用例、重新生成分支、提交评审。自由拖动仅改变视图偏好，不改变业务层级。
...
| 状态 | enum | Pending / 通过 / 不通过 / 跳过 / 堵塞 |

步骤支持测试数据、操作、预期结果、附件与自动化参数，但首屏只展开“操作 + 校验点”。

### 8.5 用例列表

- 默认列：编号、名称、模块、类型、优先级、状态、来源。
- 支持按需求、测试点、生成批次、风险、评审人和自动化状态筛选。
- 批量操作包括：提交评审、设优先级、添加标签、AI 改写、导出；删除已通过用例需确认。
- 列表保存个人视图，不改变空间公共字段。
...
- 顶部选择当前用例集合并展示本次执行进度。
- 左侧是执行队列，显示编号、优先级、模块和执行结果。
- 右侧展示只读用例内容、步骤确认、实际结果和缺陷引用。
- QA 可标记未执行、通过、不通过、跳过或堵塞。
- 用例内容状态只读展示，执行操作不修改用例 Revision。
...
- **FR-304** 已通过用例被修改时创建新的 Candidate Revision，新 Revision 的用例状态为 Pending，并保留原 Published Revision。
...
- **FR-907** 执行结果不得覆盖 `case_status` 或隐式创建用例 Revision。
...
`Pending → 通过 / 不通过 / 跳过 / 堵塞`
`不通过 → Pending`：修改内容后重新确认。
`跳过 → Pending`：重新纳入范围。
`堵塞 → Pending`：依赖恢复或问题解除。
`通过 → Pending`：创建新修订时发生，旧的已通过 Revision 保持不可变。

评审分配、评论、通过/驳回动作保存为事件，不再形成第二套与 `case_status` 冲突的用户可见状态。
...
12. 已通过用例被修改时，旧 Published Revision 仍可查看，新 Candidate Revision 的状态回到 Pending。
...
当前 `prototype/` 是前端交互原型，使用模拟数据和本地状态，已实现：

- 三栏工作台与全局导航。
- 空间管理与空间切换、新建、编辑、删除演示。
- 用例集合查看、新建、搜索、编辑、删除和最近用例视图。
- Excel 历史用例上传、字段映射、重复统计和导入结果演示。
- AI 对话、附件入口和分阶段生成演示。
- 可缩放、可选择的测试点/用例脑图。
- 用例列表与右侧结构化详情。
- AI 改写候选、差异预览、接受与撤回。
- 测试说明文档预览及 Markdown/JSON 下载。
- 本地账号登录、注册、会话恢复与退出登录。
- 响应式降级与减少动效支持。

原型不包含真实模型调用、后端持久化、真实账号与邮件服务、文件解析、评审通知和 MCP Server；这些能力应在完成本轮产品评审后进入技术设计。
