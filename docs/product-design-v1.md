...
- 五种用例状态统一为 Pending、通过、不通过、跳过、堵塞，并使用文字、圆点和颜色共同表达。
- 工作区不再持续展示“本地服务已连接”；服务健康信息进入设置或故障提示，仅在异常时打扰用户。
- 脑图缩放、状态图例、对话发送和详情提交评审保留底部安全间距。
- 发送按钮统一为低对比度深灰圆形按钮和向上箭头，首页与集合工作台行为一致。
- 当前自部署验收采用本地账号登录；公开版是否改为无登录墙仍作为后续产品决策，不与当前验收流程混写。
- 新增轻量测试执行页：QA 选择用例集合、逐条确认步骤并记录本次执行结果。
- 用例内容状态与执行结果严格分离；执行通过或失败不会修改用例的评审状态。
...
- 所有 AI 生成内容默认处于 `Pending`，人工处理后才进入其他状态。
...
- 工作台：当前会话、需求文件、生成结果、五种用例状态和评审事件。
...
| 节点 | 展示内容 | 主要操作 |
|---|---|---|
| 用例集 | 名称、用例总数、状态分布 | 全部展开、覆盖检查、发布 |
| 需求模块 | 模块名称、来源、用例数 | 补充测试点、重生成分支 |
| 测试点 | 场景、风险等级、覆盖类型 | 添加用例、合并、接受 |
| 测试用例 | 名称、优先级、用例状态 | 查看、编辑、设置状态、提交评审 |
| 需求疑点 | 疑点、影响范围、来源 | 回答、忽略、转为测试假设 |

### 7.3 节点视觉状态

- 灰色虚线节点：正在生成，不属于五种正式用例状态。
- 黄色圆点与时钟/待处理标识：Pending。
- 绿色圆点与勾：通过。
- 红色圆点与叉：不通过。
- 紫色圆点与跳过图标：跳过。
- 橙色圆点与阻断图标：堵塞。
- 需求疑点和生成质量问题使用独立风险标记，不替代用例状态。

颜色不能成为唯一状态标识，必须同时使用图标和文字。状态组件统一使用浅色背景、彩色文字和细边框，避免深色实心色块抢占页面视觉层级。
